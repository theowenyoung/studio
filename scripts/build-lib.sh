...
detect_environment() {
  ...
  if [ "${LOCAL_DEV:-}" = "true" ]; then
    ...
    export CTX_ROOT_DOMAIN="studio.localhost"
...
#   - DEPLOY_ROUTES.txt: domain|port 格式（供 Caddy 配置生成）
...
generate_deploy_summary() {
  ...
  if [ ! -f "$env_file" ]; then
    if [ -n "${PUBLIC_URL:-}" ]; then
      ...
      # 提取 domain，默认端口 3000
      local domain=$(echo "$PUBLIC_URL" | sed 's|^https://||')
      echo "${domain}|3000" > "$routes_file"
  ...
  while IFS='=' read -r key value; do
    ...
    # 提取 domain (去掉 https://)
    local domain=$(echo "$value" | sed 's|^https://||')

    # 写入文件
    echo "$value" >> "$summary_file"
    echo "${domain}|${port}" >> "$routes_file"

    echo "  📍 $value -> port $port"
