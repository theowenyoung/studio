...
if [ -n "$S3_BUCKET" ] && [ -n "$AWS_ACCESS_KEY_ID" ]; then
    echo "☁️  S3 Backups (s3://$S3_BUCKET):"
    echo ""
    ...
    aws s3 ls "s3://${S3_BUCKET}/postgres/" $AWS_ARGS 2>/dev/null | tail -10 | awk '{print "  " $0}' || echo "  No backups found or S3 access error"
    
    echo ""
    echo "Redis (last 10 days):"
    aws s3 ls "s3://${S3_BUCKET}/redis/" $AWS_ARGS 2>/dev/null | tail -10 | awk '{print "  " $0}' || echo "  No backups found or S3 access error"
else
